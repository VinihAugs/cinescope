...
function onKeydown(e: KeyboardEvent) {
  if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
    const idx = tabs.indexOf(active.value);
...
<template>
  <div class="min-h-screen pb-32 pt-24">
    ...
    >
      <div class="max-w-7xl mx-auto flex items-center justify-between gap-6">
        ...
        <div class="flex items-center gap-3">
          <RouterLink
            to="/assistir-depois"
            class="hidden sm:inline text-sm font-bold text-neutral-300 hover:text-white transition-colors"
          >
            Assistir Depois
          </RouterLink>
    ...
    <main class="max-w-7xl mx-auto px-6">
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="flex items-center gap-2">
              <span
                class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-neutral-900 border border-neutral-800 text-red-500 font-black"
                aria-hidden="true"
              >
                &lt;/&gt;
              </span>
              <h1 class="text-2xl md:text-3xl font-black tracking-tight">Bastidores (Dev Mode)</h1>
            </div>
            <p class="text-sm text-neutral-400 mt-3 leading-relaxed max-w-3xl">
      ...
      <div class="sticky top-[84px] z-30 bg-neutral-950/90 backdrop-blur-lg border border-white/10 rounded-2xl p-3 shadow-xl shadow-black/20">
      ...
      <div class="mt-5 space-y-4">
        ...
        >
          <h2 class="text-base md:text-lg font-black mb-3">{{ entry.title }}</h2>
...
<span v-for="(tok, i) in highlightTs(entry.codeSnippet)" :key="i" :class="tok.cls">{{ tok.text }}</span>
            ...
            <div>
              <p class="text-neutral-400 text-xs font-black uppercase tracking-widest mb-1">Trade-offs</p>
              <p class="text-neutral-200">
    ...
    <footer class="fixed left-0 right-0 bottom-0 z-30 border-t border-white/10 bg-neutral-950/80 backdrop-blur-lg">
      <div class="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between text-xs">
        ...
        <p class="text-neutral-600">Dev Mode</p>
      </div>
    </footer>
  </div>
</template>
